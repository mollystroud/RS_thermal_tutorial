...
This code provides an example of accessing a SpatioTemporal Asset
...
package in R, and we use gdalcubes to handle the data. We also load in
some geospatial and plotting packages.
...
feel free to put in your own area!
...
stac_search function! We will use the Landsat Collection 2 Level 2
Science Product. For more info, see:
<https://www.usgs.gov/landsat-missions/landsat-collection-2-level-2-science-products>.
We can access the STAC to see what Landsat imagery is available, and we
also filter out scenes with high cloud cover.
...
spatial and temporal resolutions, so we don't download more data than we
are interested in.
...
```{r define cube space & collection, message = FALSE}
...
cube <- cube_view(srs = "EPSG:4326",
                  ...
                  aggregation = "median", # handle pixels containing data from multiple images
                  resampling = "average") # resampling method for gdalwarp
...
cube' that loads in the data from the image collection and cube view.
Then, before downloading the image, we rename the thermal band to a more
intuitive name and rescale the raw thermal band values to degrees
Celsius.

Then, we download the data from the cloud by reading it in as a stars
object, which is a kind of data cube. It may take a few minutes to
create the stars object, since we are moving the remote sensing imagery
